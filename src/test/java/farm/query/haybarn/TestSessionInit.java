...
import static farm.query.haybarn.JdbcUtils.bytesToHex;
...
public class TestSessionInit {
    ...
    public static void test_session_init_db_only() throws Exception {
        try (TempDirectory td = new TempDirectory()) {
            ...
            try (Connection conn = DriverManager.getConnection("jdbc:haybarn:;session_init_sql_file=" + initSqlFile);
        ...
        try (TempDirectory td = new TempDirectory()) {
            ...
            Files.write(initSqlFile, ("CREATE TABLE tab1(col1 int);\n"
                                      ...
                                      + "INSERT INTO tab1 VALUES(42);")
                                         .getBytes());
            String url = "jdbc:haybarn:memory:test1;session_init_sql_file=" + initSqlFile;
    ...
    public static void test_session_init_sha256() throws Exception {
        try (TempDirectory td = new TempDirectory()) {
            ...
            try (Connection conn = DriverManager.getConnection("jdbc:haybarn:;"
            ...
            assertThrows(() -> {
                DriverManager.getConnection("jdbc:haybarn:;"
                                            + "session_init_sql_file=" + initSqlFile + ";"
    ...
    public static void test_session_init_tracing() throws Exception {
        ...
        try (TempDirectory td = new TempDirectory()) {
            ...
            try (DuckDBConnection conn =
                     DriverManager.getConnection("jdbc:haybarn:;session_init_sql_file=" + initSqlFile)
    ...
    public static void test_session_init_invalid_params() throws Exception {
        try (TempDirectory td = new TempDirectory()) {
            Path initSqlFile = td.path().resolve("init.sql");
            Files.write(initSqlFile, "CREATE TABLE tab1(col1 int);".getBytes());
            DriverManager.getConnection("jdbc:haybarn:;session_init_sql_file=" + initSqlFile).close();
            assertThrows(() -> {
                DriverManager.getConnection("jdbc:haybarn:;"
                                            + "threads=1;"
                                            + "session_init_sql_file=" + initSqlFile);
            ...
            assertThrows(() -> {
                ...
                DriverManager.getConnection("jdbc:haybarn:;", config);
            }, SQLException.class);
        }
    }
