...
import static farm.query.haybarn.HaybarnDriver.HAYBARN_URL_PREFIX;
import static farm.query.haybarn.HaybarnDriver.MEMORY_DB;
...
final class JdbcUtils {
    ...
    static String dbNameFromUrl(String url) throws SQLException {
        ...
        if (!url.startsWith(HAYBARN_URL_PREFIX)) {
            throw new SQLException("Haybarn JDBC URL needs to start with 'jdbc:haybarn:'");
        }
        ...
        String dbName = shortUrl.substring(HAYBARN_URL_PREFIX.length()).trim();
        if (dbName.length() == 0) {
            dbName = MEMORY_DB;
        }
