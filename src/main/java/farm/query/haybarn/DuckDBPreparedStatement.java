...
import static farm.query.haybarn.StatementReturnType.*;
import static farm.query.haybarn.io.IOUtils.*;
...
import farm.query.haybarn.user.DuckDBUserArray;
...
public class DuckDBPreparedStatement implements PreparedStatement {
    ...
    private void scheduleCancelTask() {
        if (queryTimeoutSeconds <= 0 || HaybarnDriver.scheduler.isShutdown()) {
            return;
        }
        cleanupCancelQueryTask();
        try {
            this.cancelQueryFuture =
                HaybarnDriver.scheduler.schedule(new CancelQueryTask(), queryTimeoutSeconds, SECONDS);
