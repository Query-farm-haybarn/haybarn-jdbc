...
import static farm.query.haybarn.JdbcUtils.*;
import static farm.query.haybarn.io.IOUtils.readToString;
...
import farm.query.haybarn.DuckDBFunctions.RegisteredFunction;
import farm.query.haybarn.io.LimitedInputStream;

public class HaybarnDriver implements java.sql.Driver {
    ...
    static final String HAYBARN_URL_PREFIX = "jdbc:haybarn:";
    static final String MEMORY_DB = ":memory:";
    private static final String DUCKLAKE_URL_PREFIX = HAYBARN_URL_PREFIX + "ducklake:";
    ...
    private static final String SESSION_INIT_SQL_FILE_URL_EXAMPLE =
        "jdbc:haybarn:/path/to/db1.db;session_init_sql_file=/path/to/init.sql;session_init_sql_file_sha256=...";
    ...
    static {
        try {
            DriverManager.registerDriver(new HaybarnDriver());
    ...
    public boolean acceptsURL(String url) throws SQLException {
        return null != url && url.startsWith(HAYBARN_URL_PREFIX);
    }
    ...
    private static void removeUnsupportedOptions(Properties props) throws SQLException {
        ...
        try {
            if (supportedOptions.isEmpty()) {
                Driver driver = DriverManager.getDriver(HAYBARN_URL_PREFIX);
                Properties dpiProps = new Properties();
                dpiProps.put("threads", 1);
                DriverPropertyInfo[] dpis = driver.getPropertyInfo(HAYBARN_URL_PREFIX, dpiProps);
